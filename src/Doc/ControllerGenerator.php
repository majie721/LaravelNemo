...
class ControllerGenerator extends BaseCodeGenerator
{
    ...
    {
//        if($this->methodInfo->action ==='edit'){
//            dd($this);
//        }
        ...
        if($res){ //已有文件
            ...
            if(false === $hasService || $this->responseClass || $this->paramBeanClass || $this->entityclass){ //如果没有注入 php文件要加入use语句, 构造函数注入service
                $flag = 0;
                $postion = 0;
            ...
            $lines[] = "use \Illuminate\Http\JsonResponse;";
    ...
    public function saveContent($varName){
        ...
        $lines[] = "{$tab}public function {$this->actionName}({$this->paramBeanName} \$bean): JsonResponse {";
    ...
    public function deleteContent($varName): array
    {
        ...
        $lines = [];
        $lines[] = "{$tab}#[ApiDoc('','',)]";
        $lines[] = "{$tab}public function {$this->actionName}(int \$id): JsonResponse{";
        $lines[] = "{$tab}{$tab}\$this->{$varName}->{$this->actionName}(\$id);";
        $lines[] = "{$tab}{$tab}return ApiResponse::success();";
        $lines[] = "{$tab}}";
        return $lines;
    }
    ...
    public function queryPaginateContent($varName): array
    {
        ...
        $lines[] = "{$tab}public function {$this->actionName}({$this->paramBeanName} \$bean): JsonResponse{";
        $lines[] = "{$tab}{$tab}\$result = \$this->{$varName}->{$this->actionName}(\$bean);";
        $lines[] = "{$tab}{$tab}return ApiResponse::data({$this->responseClassName}::fromItem(\$result));";
        $lines[] = "{$tab}}";
        return $lines;
    }
    ...
    public function queryAllContent($varName): array
    {
        ...
        $lines[] = "{$tab}public function {$this->actionName}({$this->paramBeanName} \$bean): JsonResponse{";
        $lines[] = "{$tab}{$tab}\$result = \$this->{$varName}->{$this->actionName}(\$bean);";
        $lines[] = "{$tab}{$tab}return ApiResponse::data({$this->responseClassName}::fromItem(['data'=>\$result]));"; //::fromItem(['data'=>$result])
        $lines[] = "{$tab}}";
        return $lines;
    }
